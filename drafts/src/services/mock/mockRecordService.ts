...
import type { CreateRecordInput, PageResult, RecordEntity, RecordListQuery, UpdateRecordPatch } from "../../types/index";

import { createMockId, stripMarkdown } from "../../mock/index";

export class MockRecordService implements RecordService {
  ...
  async updateRecord(id: string, patch: UpdateRecordPatch): Promise<RecordEntity> {
    ...
    if (patch.contentMarkdown !== undefined && patch.contentMarkdown !== null) {
      nextPatch.contentMarkdown = patch.contentMarkdown;
    }
  ...
  async batchReschedule(ids: string[], strategy: Parameters<RecordRepository["batchReschedule"]>[1]): Promise<RecordEntity[]> {
